...
pub fn main() !void {
    ...
    const rx = try wf.makeDragF32(hit_test_widget.circle.rx, ids.rx);
    ...
    while (!window.closed()) {
        ...
        for (gui_state.event_queue.items) |event| switch (event) {
            ids.rx => {
                try rx.update(&hit_test_widget.circle.rx, 0.001);
                hit_test_widget.updateRender();
            },
            ids.ry => {
                try ry.update(&hit_test_widget.circle.ry, 0.001);
                hit_test_widget.updateRender();
            },
            ids.rot => {
                try rot.update(&hit_test_widget.circle.rotation, 0.001);
                hit_test_widget.updateRender();
            },
            else => {},
        };
